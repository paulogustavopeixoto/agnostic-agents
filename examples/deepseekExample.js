// examples/deepseekExample.js
const { Agent, Tool, DeepSeekAdapter, MCPTool } = require('../index');
...
(async () => {
  ...
  try {
    ...
    const agentWithTools = new Agent(deepseek, {
      ...
      defaultConfig: { model: 'deepseek-chat', temperature: 0.7, maxTokens: 1024 },
    });
    ...
    const agentSimple = new Agent(deepseek, {
      ...
      defaultConfig: { model: 'deepseek-chat', temperature: 0.8, maxTokens: 512 },
    });
    ...
    try {
      const embeddings = await deepseek.embedChunks(textChunks, {
        model: 'text-embedding', // Adjust if DeepSeekAdapter uses a specific model
      });
      ...
      console.warn('Embeddings not supported by DeepSeekAdapter:', error.message);
    }
